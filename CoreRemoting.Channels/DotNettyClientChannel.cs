...
namespace CoreRemoting.Channels.DotNetty
{
    ...
    public class DotNettyClientChannel : IClientChannel, IRawMessageTransport
    {
        ...
        public async Task ConnectAsync()
        {
            ...
            _channel = await _bootstrap.ConnectAsync(new IPEndPoint(IPAddress.Parse(_serverHostName), _serverPort))
                .ConfigureAwait(false);

            // Send handshake metadata
            var handshakeData = Encoding.UTF8.GetBytes(System.Text.Json.JsonSerializer.Serialize(_handshakeMetadata));
